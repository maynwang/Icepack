...
! with the GEM data
       IF(kt == 1 ) CALL test_input_cice(kt)
       IF(kt == 10) CALL test_input_cice(kt)
	print *, 'END DATA MANIP. FOR KT = ', kt
...
!      type(datetime_type), intent(in) :: date
	...
	print *, 'writing outputs of sfc fluxes'
      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/uatm" &
                        ,i2.2)') kt
      open (80, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/vatm" &
                        ,i2.2)') kt
      open (81, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/wind" &
                        ,i2.2)') kt
      open (82, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/fsw" &
                        ,i2.2)') kt
      open (83, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/flw" &
                        ,i2.2)') kt
      open (84, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/Tair" &
                        ,i2.2)') kt
      open (85, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/rhoa" &
                        ,i2.2)') kt
      open (86, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/Qa" &
                        ,i2.2)') kt
      open (87, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/zlvl" &
                        ,i2.2)') kt
      open (88, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/swvdr" &
                        ,i2.2)') kt
      open (89, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/swvdf" &
                        ,i2.2)') kt
      open (90, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/swidr" &
                        ,i2.2)') kt
      open (91, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/swidf" &
                        ,i2.2)') kt
      open (92, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/fsnow" &
                        ,i2.2)') kt
      open (93, file = filename, status = 'unknown')

      write (filename,'("/home/map005/data/eccc-ppp2/CICE_atm_input/frain" &
                        ,i2.2)') kt
      open (94, file = filename, status = 'unknown')


      do j = 1, jpj
         write(80,100) ( uatm_rpn(i,j),    i = 1, jpi )
         write(81,100) ( vatm_rpn(i,j),    i = 1, jpi )
         write(82,200) ( wind_rpn(i,j),    i = 1, jpi )
         write(83,200) ( fsw_rpn(i,j),     i = 1, jpi ) 
	 write(84,100) ( flw_rpn(i,j),     i = 1, jpi )
         write(85,100) ( Tair_rpn(i,j),    i = 1, jpi )
         write(86,100) ( rhoa_rpn(i,j),    i = 1, jpi )
         write(87,100) ( Qa_rpn(i,j),      i = 1, jpi )
         write(88,100) ( zlvl_rpn(i,j),    i = 1, jpi )
         write(89,100) ( swvdr_rpn(i,j),   i = 1, jpi )
         write(90,100) ( swvdf_rpn(i,j),   i = 1, jpi )    
         write(91,100) ( swidr_rpn(i,j),   i = 1, jpi )
         write(92,100) ( swidf_rpn(i,j),   i = 1, jpi )
         write(93,100) ( fsnow_rpn(i,j),   i = 1, jpi )
         write(94,100) ( frain_rpn(i,j),   i = 1, jpi )          
      enddo
...
300            format (1x, 1000(f20.4,  1x))
      ...
      return
    END SUBROUTINE test_input_cice   
